...
def Forward_Model(W, F_outgass, n, climp, tdep_weath, mod_sea, alt_frac, 
                  Mp_frac, lfrac, carb_exp, sed_thick, F_carbw, fpel, CWF, deep_grad, 
    ...
    options_array = np.load('options_array.npy')
    ...
    # initial seafloor dissolution flux i.e. dissolution = alt_frac*precip_seafloor
    F_diss0 = alt_frac*partition*F_outgass
    # initial continental silicate weathering flux
    F_sil0 = (1-partition)*F_outgass+(1-alt_frac)*partition * F_outgass  
    # initial pore space carbonate precipation flux (reduntant)
    Fprecip_p = partition*F_outgass
    ...
    [vv, xx] = pylab.roots(
        [ALK_p/(10**-pK1*10**-pK2), (ALK_p-DIC_p)/(10**-pK2), ALK_p-2*DIC_p])  # equation S15
    H_p = np.max([vv, xx])  # take positive root
    ...
    # only true for high pH, okay for fitting I think (actually should include CO2aq to get exact).
    co3_p = ALK_p-DIC_p-H_p+CO2aq_p
    # Modern bicarbonate alkalinity in pore space (not used)
    hco3_p = co3_p*10**-pH_p/(10**-pK2)
    ...
    # no pelagic carbonates in this version of the code (i.e. default is fpel=0)
    frac_pel = fpel
    ko1 = (1-frac_pel)*Fprecip_o/(omega_o - 1)**n
    ko2 = frac_pel*Fprecip_o/(omega_o**2.84)

    # remaining proportionality constants
    k_r = F_diss0/(2.88*10**-14*10**(-coef_for_diss*pH_p)*np.exp(-Ebas /
                   (8.314*(buffer_T+Pore_mod))))  # for seafloor dissolution
    ...
    [out, mes] = scipy.integrate.odeint(system_of_equations, [DIC_o+1.8e20/Mo*ppCO2_o, ALK_o, DIC_p, ALK_p], time, args=(W, F_outgass, n, climp, tdep_weath, mod_sea,
                                        alt_frac, Mp_frac, lfrac, carb_exp, sed_thick, F_carbw, CWF, deep_grad, coef_for_diss, beta, n_out, mm, growth_timing, new_add_Ca, Ebas, f), full_output=1)
    ...
    R_ratio = 0 * time  # Weather rate ratio
    temp_e = np.ones(len(time)) * tdep_weath
    carb_e = np.ones(len(time)) * carb_exp
    bio_factor = 0 * time
    ...
    for i in range(0, len(time)):
        ...
        pl1 = carbon_cycle([out[i, 0], out[i, 1], out[i, 2], out[i, 3]], time[i], W, F_outgass, n, climp, tdep_weath, mod_sea, alt_frac, Mp_frac,
                           lfrac, carb_exp, sed_thick, F_carbw, CWF, deep_grad, coef_for_diss, beta, n_out, mm, growth_timing, new_add_Ca, Ebas, f)  # ocean
    ...
    max_el = np.size(time)-1  # number of elements in time array
    tstep = np.max(time)/max_el  # time step (yrs) in time array

    # calculate mass imbalance
    # integral of carbon inputs minus outpus over all timesteps
    flux_bal = np.sum(volc_array[2:]+carbw_ar[2:])*tstep - \
        np.sum(Precip_ocean_array[2:]+Precip_pore_array[2:])*tstep
    res_change = 1.8e20*(CO2_array_o[max_el]-CO2_array_o[2])+Mo*(HCO3_array_o[max_el]+CO3_array_o[max_el]+co2_aq_o[max_el]-co2_aq_o[2]-CO3_array_o[2]-HCO3_array_o[2])+Mp*(
        HCO3_array_p[max_el]+CO3_array_p[max_el]+co2_aq_p[max_el]-co2_aq_p[2]-CO3_array_p[2]-HCO3_array_p[2])  # final - initial carbon abundances (starting at second element to avoid transients)
    # alternative way of calculating final - initial carbon abundances
    ...
    return [out[:, 0], out[:, 1], out[:, 2], out[:, 3], time, pH_array_o, CO2_array_o,
            pH_array_p, CO2_array_p, Ca_array_o, Ca_array_p, CO3_array_o, CO3_array_p,
            HCO3_array_o, HCO3_array_p, omega_o, omega_p, Tsurf_array, Tdeep_array,
            Fd_array, Fs_array, Precip_ocean_array, Precip_pore_array, spread_ar,
            volc_array, T_pore_space_ar, R_ratio, temp_e, carb_e, carbw_ar, bio_factor], imbalance
...
def system_of_equations(y, t0, W, F_outgass, n, climp, tdep_weath, mod_sea, alt_frac, Mp_frac, lfrac, carb_exp, sed_thick, F_carbw, CWF, deep_grad, coef_for_diss, beta, n_out, mm, growth_timing, new_add_Ca, Ebas, f):
    # print('system equation',type(t0))
    ...
    # Correct dissolved inorganic carbon content of ocean by subtracting atmospheric pCO2 from atmosphere-ocean reservoir (this ensures mass balance)
    DICo = y[0]-EE_ppCO2_o*s

    # Calculate time derivatives from current state (equation S1).
    # Time derivative of atmosphere-ocean carbon abundance
    dy0_dt = F_outg/Mo+carb_weath/Mo-W*(DICo-y[2])/Mo-Precip_ocean/Mo
    # Time derivative of ocean alkalinity
    dy1_dt = 2*carb_weath/Mo-W*(y[1]-y[3])/Mo+2*F_silic/Mo-2*Precip_ocean/Mo
    # Time derivative of pore space carbon abundance
    dy2_dt = W*(DICo-y[2])/Mp-Precip_pore/Mp
    dy3_dt = W*(y[1]-y[3])/Mp+2*F_diss/Mp-2*Precip_pore / \
        Mp  # Time derivative of pore space alkalinity
...
def lf(t0, lfrac, growth_timing, f):
    # LL=1/(1-lfrac)
    # land_frac=np.max([0.0,1-1/(LL+np.exp(-10*(t0/1e9-growth_timing)))]) # max needed to prevent negative land fractions
    return f(t0)
...
def carbon_cycle(y, t0, W, F_outgass, n, climp, tdep_weath, mod_sea, alt_frac, Mp_frac, lfrac, carb_exp, sed_thick, F_carbw, CWF, deep_grad, coef_for_diss, beta, n_out, mm, growth_timing, new_add_Ca, Ebas, f):

    # Ca_cofactor is only important for the sensitivity tests where a wide range of Archean Ca abundances are imposed
    # perscribed ocean Ca molality evolution
    Ca_cofactor = new_add_Ca/1000.0*np.exp(t0/1e9-4)

    global pK1, pK2, H_CO2, salt, Pore_mod, T_surface0, ALK_o, ALK_p, Ca_p, Ca_o, options_array
    # ,Mg_fun,Ca_Tfun,coef_for_diss
    global landf_vary, lum_vary, salt0, k_w, k_c, k_o, k_r, Mp, ppCO2_o, Mo, ko1, ko2
    ...
    # Filler value (T_surface will be calcauled within the loop)
    T_surface = 100.0
    ...
    for jk in range(0, max_temp_iter):
        ...
        [c, d] = pylab.roots([y[1]/(10**-pK2*10**-pK1)*(1+s/H_CO2),
                             (y[1]-y[0])/(10**-pK2), (y[1]-2*y[0])])  # equation S15
        EE_H_o = np.max([c, d])  # make sure get right root!
        ...
        [c, d] = pylab.roots(
            [y[3]/(10**-pK2*10**-pK1), (y[3]-y[2])/(10**-pK2), (y[3]-2*y[2])])  # equation S15
        EE_H_p = np.max([c, d])  # make sure get right root!
        ...
        if landf_vary:
            # land = f(t0)
            land = lf(t0, lfrac, growth_timing, f)
        else:
            land = 1.0

        if lum_vary:
            # Evolution of solar luminosity from Gough 1981.
            L_over_Lo = 1/(1+0.4*(t0/4.6e9))
            # Check to see if methane option is on. If yes, use methane climate models:
            if options_array[2] == 1:
                ...
                w0 = 1-1/(1+np.exp(-15*(t0/1e9-0.541)))
                # 1 in the Proterozoic, 0 elsewhere
                w1 = 1/(1+np.exp(-15*(t0/1e9-0.541))) - \
                    1/(1+np.exp(-15*(t0/1e9-2.5)))
                # 1 in the Archean, 0 elsewhere
                w2 = 1/(1+np.exp(-15*(t0/1e9-2.5)))
                # Archean temperature with high methane
                arch_temp = clim_fun_highCH4(EE_ppCO2_o, L_over_Lo)
                # Proterozoic temperature with high methane
                Protero_temp = clim_fun_lowCH4(EE_ppCO2_o, L_over_Lo)

                # Climate model for eleveated Proterozoic and Archean methane abundances:
                T_surface = (w0*clim_fun_CO2_only(EE_ppCO2_o,
                             L_over_Lo)+w1*Protero_temp+w2*arch_temp)/(w0+w1+w2)

            # methane option off -> use CO2 only climate model (nominal model)
            else:
                T_surface = clim_fun_CO2_only(EE_ppCO2_o, L_over_Lo)
                # T_surface=clim_fun_CO2_only(EE_ppCO2_o,L_over_Lo)+30.0 * (1/(1+np.exp(-15*(t0/1e9-2.5)))) ## for +30 K sensitivity test
        else:
            # For CO2-only, no evolution in solar luminosity
            T_surface = clim_fun_CO2_only(EE_ppCO2_o, 1.0)
        ...
        # ensures deep ocean is not frozen
        buffer_Ti = np.max([deep_grad*T_surface+interc, 271.15])
        # This is deep ocean temperature (cannot be warmer than the surface)
        buffer_T = np.min([buffer_Ti, T_surface])
    ...
    EE_omega_o = act_o*EE_Ca_o * EE_co3_o / \
        (Sol_prod(T_surface))  # saturation state ocean
    EE_omega_p = act_p*EE_Ca_p * EE_co3_p / \
        (Sol_prod(T_pore_space))  # saturation state pore space

    # Calculate carbonate sinks
    # ocean carbonate from equation S19, noting that ko2 is 0 in this version of the model
    Carb_sink = ko1*land*(EE_omega_o - 1)**n+ko2*(EE_omega_o**2.84)
    Precip_ocean = Carb_sink  # Precipitaion of carbonate in ocean
    # Precipitation of carbonate in pore space
    Precip_pore = k_c*(EE_omega_p - 1)**n

    # Carbonate weathering flux
    carb_weath = F_carbw*biology_modifier * \
        (EE_ppCO2_o/ppCO2_o)**(carb_exp) * \
        np.exp((T_surface_diff)/tdep_weath)*land  # equation S2

    # Dissolution of seafloor flux:
    F_diss = k_r*2.88*10**-14*10**(-coef_for_diss*EE_pH_p)*spread**beta * \
        np.exp(-Ebas/(8.314*(T_pore_space)))  # equation S3
    # F_diss=k_r*2.88*10**-14*10**(-coef_for_diss*8.57)*np.exp(-Ebas/(8.314*(283))) # sensitivity test holding dissolution artifically constant

    # Continetnal silicate weathering flux
    F_silic = k_w*biology_modifier*land * \
        (EE_ppCO2_o/ppCO2_o)**climp * \
        np.exp((T_surface_diff)/tdep_weath)  # equation 1

    # Silicate weathering rate ratio = Rt / Rmodern
    R_ratio = biology_modifier*(EE_ppCO2_o/ppCO2_o)**climp * \
        np.exp((T_surface_diff)/tdep_weath)  # equation from jihua

    # return selected carbon cycle variables and fluxes
    return [EE_pH_o, EE_co3_o, EE_hco3_o, EE_co2aq_o, EE_ppCO2_o, EE_Ca_o, EE_omega_o, T_surface, 
    buffer_T, F_diss, F_silic, Precip_ocean, Precip_pore, EE_omega_p, F_outg, carb_weath, EE_co3_p, 
    EE_hco3_p, EE_co2aq_p, EE_pH_p, EE_ppCO2_p, EE_Ca_p, EE_H_o, EE_H_p, T_surface_diff, F_outg, 
    spread, T_pore_space, R_ratio, biology_modifier]  # T_pore_space
